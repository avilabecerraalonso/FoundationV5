...
$sqlfe = "SELECT * FROM users WHERE cookie = '$cookieuser'";
$resultfe = mysqli_query($conn, $sqlfe);

if ($resultfe && mysqli_num_rows($resultfe) > 0) {
    $user_infofe = mysqli_fetch_assoc($resultfe);
    $namefe = $user_infofe['name'];
    $levelfe = $user_infofe['level'];
    if ($levelfe!=='1') {
...
// Retrieve search data from POST request
$page = $_POST['page'];
$keyword = $_POST['keyword'];

// Calculate start and limit values for SQL query
$start = ($page - 1) * $results_per_page;
$limit = $results_per_page;

// Build SQL query based on search keyword
$sql = "SELECT * FROM users";
if (!empty($keyword)) {
	$sql .= " WHERE name LIKE '%$keyword%' OR lastname LIKE '%$keyword%'";
}
...
if ($result->num_rows > 0) {
	...
	$output = "<table class='table table-hover' stile='border-radius:25px;'>
					...
					<tr class='backgroundalt'>
						<th class='col-9' scope='col'>Nombre</th>
	...
	// Output table footer and pagination
	$output .= "</tbody></table>";
