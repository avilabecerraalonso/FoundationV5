...
if(isset($_GET['id'])) {
    $idnew = $_GET['id'];
    $sqlnew = "SELECT * FROM news WHERE id = '$idnew'";
$resultnew = mysqli_query($conn, $sqlnew);


if ($resultnew && mysqli_num_rows($resultnew) > 0) {
    $newinfo = mysqli_fetch_assoc($resultnew);
    $title = $newinfo['title'];
    $content = $newinfo['content'];
    $url = $newinfo['url'];
    $category = $newinfo['category'];
    $datenew = $newinfo['creadted_at'];
} else {
...
<head>
  ...
  <title><? echo $title; ?> | Fundación Jóvenes Soñadores Por Talaigua</title>
...
<body class="mb-5">
  ...
  <div class="conatiner-fluid p-4 text-center">
  <h2 class="mt-3"><? echo $title; ?></h2>
    <? if($url!==''){ echo'<img src="./assets/images/news/'.$url.'" class="img-fluid rounded" alt="'.$title.'"/>'; } ?>
  </div>
  <div class="conatiner-fluid pt-0 p-4">
    <? echo $content; ?>
  </div>
