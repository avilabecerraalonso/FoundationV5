...
<head>
    ...
    <title>Donaciones | Fundación Jóvenes Soñadores Por Talaigua</title>
...
<body class="mb-5">
    ...
    <div class="conatiner-fluid p-4">
        ...
        $sql = "SELECT * FROM `donations` LIMIT $offset, $results_per_page";
        $result = mysqli_query($conn, $sql);
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            $lastLoginTime = strtotime($row["updated_at"]);
            ...
            $target = $row["target"];
            $percentage = ($current_amount / $target) * 100;
            $formatted_percentage = number_format($percentage, 2, '.', '');

            echo '<div class="card mb-3">';
            echo '<div class="row g-0">';
            echo '<div class="col-4">';
            echo '<img src="' . $row["image"] . '" class="img-fluid rounded-start img-cover" alt="' . $row["title"] . '" style="height: auto; object-fit: cover;">';
            echo '</div>';
            echo '<div class="col-8">';
            echo '<div class="card-body">';
            echo '<h6 class="card-title">' . $row["title"] . '</h6>';
            echo '<p class="card-text text-truncate">' . $row["description"] . '</p>';
            echo '<div class="progress" role="progressbar" aria-label="Example 4px high" aria-valuenow="' . $percentage . '" aria-valuemin="0" aria-valuemax="100" style="height: 3px">';
            echo '<div class="progress-bar background10" style="width: ' . $percentage . '%"></div>';
            echo '</div>';
            echo '<div class="d-flex justify-content-between align-items-center">';
            echo '<p class="mb-0"><small class="text-body-secondary mb-0">Recaudado: $' . number_format($row["current_amount"], 0, '.', ',') . '</small></p>';
            echo '<p class="mb-0"><small class="text-body-secondary mb-0">Meta: $' . number_format($row["target"], 0, '.', ',') . '</small></p>';
            echo '</div>';
            echo '<p class="card-text"><small class="text-body-secondary">Última actualización: ' . $time_ago . '</small></p>';
            echo '</div>';
            echo '</div>';
            echo '</div>';
            echo '</div>';
        ...
        if($total_pages>1){
        echo '<nav class="pagdon" aria-label="Page navigation">';
        echo '<ul class="pagination justify-content-center">';
        for ($i = 1; $i <= $total_pages; $i++) {
            echo '<li class="page-item ' . ($page == $i ? ' active' : '') . '"><a class="page-link" ' . ($page == $i ? ' style="color: #1F2D36 !important; text-decoration: none; border: #BDBFC4 solid 1px;"' : '') . ' href="?page=' . $i . '">' . $i . '</a></li>';
        }
        echo '</ul>';
        echo '</nav>';}
    ...
    <nav class="nav nav-fill fixed-bottom backgroundalt pb-3">
        ...
        <a class="nav-link active" aria-current="page" href="./donations"><svg xmlns="http://www.w3.org/2000/svg"
                width="100%" height="100%" fill="currentColor" class="bi bi-coin" viewBox="0 0 16 16">
                <path
