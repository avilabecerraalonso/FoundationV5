...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['titlenewnews'])) {

  $title = $_POST['titlenewnews'];
  ...
  if ($check !== false) {
      ...
      header('Location: ./news?upload=ni');
      $uploadOk = 0;
  }

  if (file_exists($target_file)) {        
      header('Location: ./news?upload=exist');
      $uploadOk = 0;
  }

  if ($_FILES["icobadget"]["size"] > 50000000) {        
      header('Location: ./news?upload=large');
      $uploadOk = 0;
  }
  ...
  if ($uploadOk == 0) {        
      header('Location: ./news?upload=n');
  } else {
      if (move_uploaded_file($_FILES["icobadget"]["tmp_name"], $target_file)) {
          $url = basename($_FILES["icobadget"]["name"]);

  $sql = "INSERT INTO news (title, url, content, category) VALUES ('$title', '$url', '$content', 'Noticia')";

  if (mysqli_query($conn, $sql)) {
      header('Location: ./news?created=y');
  } else {
      echo 'Error: ' . mysqli_error($conn);
  }
      } else {
      header('Location: ./news?upload=e');
      }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['idnewsedit'])) {


  if ($_FILES['icobadget']['size'] == 0) {
        
    $title = $_POST['title'];
    $idnews = $_POST['idnewsedit'];
    $content = $_POST['content'];
        
    $sql = "UPDATE badget SET title='$title', content='$content' WHERE id=$idnews";
    if ($conn->query($sql) === TRUE) {
        header('Location: ./news?edited=y');
    }else{   
        header('Location: ./news?edited=n');
    }
    ...
    if ($uploadOk == 0) {        
        ...
        if (move_uploaded_file($_FILES["icobadget"]["tmp_name"], $target_file)) {
            ...
            $sql = "UPDATE news SET title='$title', content='$content', url='$url' WHERE id=$idnews";
            if ($conn->query($sql) === TRUE) {
                

                $filename = 'assets/images/news/' . $urlold;
            if (file_exists($filename)) {
                if (unlink($filename)) {
                    header('location: ./news?edit=yes');
                } else {
                    header('Location: ./news?editdeleted=nimage');
                }
            } else {
                header('Location: ./news?editdeleted=nfound');
            }

            
            } else {
            header('location: ./news?edit=bad');
            }
...
///after
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['idnewsdelete'])) {
  $url = $_POST['urlnewsdelete'];
  $filename = 'assets/images/news/'.$url;
if (file_exists($filename)) {
  if (unlink($filename)) {
    $ideletebad = $_POST['idnewsdelete'];
  $sql = "DELETE FROM news WHERE id = $ideletebad";
  if ($conn->query($sql) === TRUE) {
  header('Location: ./news?deleted=y');
} else {
  header('Location: ./news?deleted=n');
  }
  } else {
    header('Location: ./news?deleted=nimage');
  }
} else {
  header('Location: ./news?deleted=nfound');
}
...
<head>
  ...
  <title>Lista de Noticias | Fundación Jóvenes Soñadores Por Talaigua</title>
  ...
  <div class="conatiner-fluid p-4">
    ...
    <div class="row">
      <div class="form-group has-search col-10 pe-0">
        <input type="text" class="form-control backgroundalt searchhome" placeholder="Busca noticias..." id="keyword"
          name="keyword">
      </div>
      <div class="notifications col-2 text-center">
        <div class="icon" data-bs-toggle="modal" data-bs-target="#ModalNews">
            ...
            <div class="modal-dialog modal-dialog-centered" role="document">
                <div class="modal-content rounded-4 shadow">
                    <div class="modal-header p-5 pb-4 border-bottom-0">
                        <h1 class="fw-bold mb-0 fs-2">Nueva Noticia</h1>
                    ...
                    <div class="modal-body p-5 pt-0">
                        <form class="" method="post" action="" enctype="multipart/form-data">
                            <div class="form mb-3">
                                ...
                                <input type="text" name="titlenewnews" class="form-control rounded-3" id="floatingtitle" rows="2" required />
                            </div>
      ...
      <?php while ($rowmodalnews = mysqli_fetch_assoc($resultmodalnews)):
        ...
        <div class="modal fade" id="ModalEdit<? echo $rowmodalnews['id']; ?>" tabindex="-1"
          ...
          <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content rounded-4 shadow">
              <div class="modal-header p-5 pb-4 border-bottom-0">
                <h1 class="fw-bold mb-0 fs-2">Editar Noticia</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
              </div>

              <div class="modal-body p-5 pt-0">
                <form class="" method="post" action="" enctype="multipart/form-data">
                  <input type="text" class="visually-hidden" name="idnewsedit" value="<? echo $rowmodalnews['id']; ?>" />
                  <input type="text" class="visually-hidden" name="urlnewsedit" value="<? echo $rowmodalnews['url']; ?>" />
                  <div class="form mb-3">
                    <label class="form-label">Titulo</label>
        ...
        <div class="modal fade" id="Modaldelete<? echo $rowmodalnews['id']; ?>" tabindex="-1"
          ...
          <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content rounded-4 shadow">
              <div class="modal-header p-5 pb-4 border-bottom-0">
                <h1 class="fw-bold mb-0 fs-2">Eliminar Noticia</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
              </div>

              <div class="modal-body p-5 pt-0">
                <form class="" method="post" action="">
                  <input type="text" class="visually-hidden" name="urlnewsdelete" value="<? echo $rowmodalnews['url']; ?>" />
                  <input type="text" class="visually-hidden" name="idnewsdelete" value="<? echo $rowmodalnews['id']; ?>" />
                  <h4>La cuenta de <a class="text-reset fw-bolder text-danger-emphasis">
                      <? echo $rowmodalnews['title']; ?>
